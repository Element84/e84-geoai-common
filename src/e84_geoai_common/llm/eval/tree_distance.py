...
def get_tree_edit_distance(node1: BaseModel, node2: BaseModel) -> float:
    ...
    if node1 == node2 and distance != 0.0:
        raise RuntimeError("Expected two nodes that are equivalent to have an edit distance of 0")

    if distance == 0.0 and node1 != node2:
        raise RuntimeError("Expected two nodes to be equal with an edit distance of 0")
